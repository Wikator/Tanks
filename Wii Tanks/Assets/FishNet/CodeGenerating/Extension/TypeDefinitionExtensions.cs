...
namespace FishNet.CodeGenerating.Extension
{
    ...
    {
        public static MethodReference GetMethodReferenceInBase(this TypeDefinition td, CodegenSession session,
            string methodName)
        {
            var baseMd = td.GetMethodDefinitionInBase(session, methodName);
            if (baseMd == null)
                return null;


            MethodReference baseMr;
            var baseTr = td.BaseType;
            if (baseTr.CachedResolve(session).HasGenericParameters)
            {
                var git = (GenericInstanceType)baseTr;
                ...
                foreach (var pd in baseMd.Parameters)
                {
        ...
        /// </summary>
        public static MethodDefinition GetMethodDefinitionInBase(this TypeDefinition td, CodegenSession session,
            string methodName)
        {
            if (td.BaseType == null)
            {
            ...
            var baseTd = td.BaseType.CachedResolve(session);
        ...
        ///     Returns a method in the next base class.
        /// </summary>
        public static MethodReference GetMethodReference(this TypeDefinition td, CodegenSession session,
            string methodName)
        {
        ...
        /// </summary>
        public static MethodReference GetOrCreateMethodReference(this TypeDefinition td, CodegenSession session,
            string methodName, MethodAttributes attributes, TypeReference returnType, out bool created)
        {
            var md = td.GetMethod(methodName);
            //Not found.
            if (md == null)
        ...
        /// </summary>
        public static MethodDefinition GetOrCreateMethodDefinition(this TypeDefinition td, CodegenSession session,
            string methodName, MethodAttributes attributes, TypeReference returnType, out bool created)
        {
            var md = td.GetMethod(methodName);
            //Not found.
        ...
        /// </summary>
        public static MethodDefinition GetOrCreateMethodDefinition(this TypeDefinition td, CodegenSession session,
            string methodName, MethodDefinition methodTemplate, bool copyParameters, out bool created)
        {
            var md = td.GetMethod(methodName);
            //Not found.
            if (md == null)
            {
                var returnType = session.ImportReference(methodTemplate.ReturnType);
                ...
                if (copyParameters)
                    foreach (var pd in methodTemplate.Parameters)
        ...
        /// </summary>
        public static MethodDefinition GetMethodDefinitionInAnyBase(this TypeDefinition td, CodegenSession session,
            string methodName)
        {
            while (td != null)
            {
        ...
        {
            FieldReference fr;
            var declaringType = fd.DeclaringType;
            //Is generic.
            if (declaringType.HasGenericParameters)
            {
                var git = new GenericInstanceType(declaringType);
                foreach (var item in declaringType.GenericParameters)
        ...
        /// </summary>
        public static FieldReference GetOrCreateFieldReference(this TypeDefinition td, CodegenSession session,
            string fieldName, FieldAttributes attributes, TypeReference fieldTypeRef, out bool created)
        {
            var fr = td.GetFieldReference(fieldName, session);
            if (fr == null)
            {
        ...
        ///     Creates a FieldReference.
        /// </summary>
        public static FieldReference CreateFieldDefinition(this TypeDefinition td, CodegenSession session,
            string fieldName, FieldAttributes attributes, TypeReference fieldTypeRef)
        {
            var fd = new FieldDefinition(fieldName, attributes, fieldTypeRef);
            td.Fields.Add(fd);
