...
namespace FishNet.CodeGenerating.Helping
{
    internal class ObjectHelper : CodegenBase
    {
        public override bool ImportReferences()
        {
            ...
            var networkConnectionTr = ImportReference(tmpType);
            foreach (var item in networkConnectionTr.CachedResolve(Session).Properties)
            ...
            var dictTypeRef = ImportReference(dictType);
            //Dictionary_Add_UShort_SyncBase_MethodRef = dictTypeRef.CachedResolve(base.Session).GetMethod("add_Item", )
            foreach (var item in dictTypeRef.CachedResolve(Session).Methods)
            ...
            //NetworkConnection.
            foreach (var pi in typeof(NetworkConnection).GetProperties(BindingFlags.Static | BindingFlags.Public |
