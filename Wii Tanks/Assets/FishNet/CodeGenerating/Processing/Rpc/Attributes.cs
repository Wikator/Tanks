...
namespace FishNet.CodeGenerating.Processing.Rpc
{
    internal class Attributes : CodegenBase
    {
        ...
        public bool HasRpcAttributes(MethodDefinition methodDef)
        {
            ...
            {
                var rt = Session.GetClass<AttributeHelper>().GetRpcAttributeType(customAttribute);
                if (rt != RpcType.None)
                    return true;
            }
        ...
        public List<AttributeData> GetRpcAttributes(MethodDefinition methodDef)
        {
            ...
            var asyncAttributeFullName = typeof(AsyncStateMachineAttribute).FullName;
            var isAsync = false;
        ...
        private bool IsRpcMethodValid(MethodDefinition methodDef, RpcType rpcType)
        {
            ...
            for (var i = 0; i < methodDef.Parameters.Count; i++)
            {
                ...
                var canSerialize = GetClass<GeneralHelper>()
                    .HasSerializerAndDeserializer(parameterDef.ParameterType, true);
                if (!canSerialize)
                {
                    Session.LogError(
